...
```json
"nabarabane/amocrm": "~1.7"
```

Или введите в консоли
```sh
composer require nabarabane/amocrm:"~1.7"
```
...
Также можно указать директорию конфига при создании экземпляра класса `Handler`, например:

```php
$api = new Handler('subdomain', 'email@dmonain.com', true, __DIR__.'/config');
```
...
/* Создание экземпляра API, где:
- первый параметр, "domain" - имя вашего домена в AmoCRM;
- второй параметр, "user@example.com" - email пользователя, от чьего имени будут совершаться запросы;
- третьим параметром указывается, что включен режим отладки, по умолчанию "false"
- четвертый параметр - указывается путь к папке конфига
- пятый параметр - домен, "ru" (по умолчанию) или "com", необходимо указывать, если работа ведется с английской версией AmoCRM 
*/
$api = new Handler('domain', 'user@example.com', true, __DIR__.'/config', 'ru');
